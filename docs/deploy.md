...
2. **Add New… → Project** → Import **`pmochoki/Nuvelo`** from GitHub.
...
5. **Environment variables:** none required. The browser calls **`https://nuvelo-backend.onrender.com`** automatically when the site is not served from `localhost` or `*.onrender.com` (see `web/app.js`).

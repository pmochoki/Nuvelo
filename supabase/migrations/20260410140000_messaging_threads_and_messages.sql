-- Nuvelo messaging (matches web/src/lib/messaging.js).
-- Apply in Supabase SQL Editor or: supabase db push
-- Prerequisite: auth.users exists. No FK to listings — listing_id is text (API listing ids).
-- After apply: Database → Replication → enable for `messages` if you want realtime chat.
